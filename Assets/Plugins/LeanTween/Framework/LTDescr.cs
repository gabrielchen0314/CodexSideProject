...
public class LTDescr
{
	...
	public LTDescr setCanvasAlpha(){
		...
		this.initInternal = ()=>{
			...
			TMPro.TextMeshProUGUI uiTMPText = trans.GetComponent<TMPro.TextMeshProUGUI>();
			...
			{
				this.fromInternal.x = uiTMPText.color.a;
			}
			else
			{
...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
	...
	private static float KeepAlphaValue(Transform iObj)
	{
		LeanTweenKeepAlpha _keepAlpha = iObj.GetComponent<LeanTweenKeepAlpha>();
		if (_keepAlpha != null)
		{
			return Math.Clamp(_keepAlpha.AlphaValue, 0, 1);
		}
		...
		if(rectTransform.childCount>0){
			foreach (RectTransform child in rectTransform) {
				...
				if (uiImage != null)
				{
					...
					c.a = val * KeepAlphaValue(uiImage.transform);
					uiImage.color = c;
				}
		...
		if(useRecursion && trans.childCount>0){
			foreach (Transform child in trans) {
				...
				if (uiText != null)
				{
					...
					c.a = val * KeepAlphaValue(uiText.transform);
					uiText.color = c;
				}
				else
				{
					...
					if (TMPText != null)
					{
						...
						c.a = val * KeepAlphaValue(TMPText.transform);
						TMPText.color = c;
					}
				}
				textAlphaChildrenRecursive(child, val);
			}
			UnityEngine.UI.Text _Text = trans.GetComponent<UnityEngine.UI.Text>();
			if (_Text != null)
			{
				Color c = _Text.color;
				c.a = val * KeepAlphaValue(_Text.transform);
				_Text.color = c;
			}
			else
			{
				TMPro.TextMeshProUGUI _TMPText = trans.GetComponent<TMPro.TextMeshProUGUI>();
				if (_TMPText != null)
				{
					Color c = _TMPText.color;
					c.a = val * KeepAlphaValue(_TMPText.transform);
					_TMPText.color = c;
				}
			}
		}
	...
	*/
	public LTDescr setEase(int ieaseType) {
		return setEase((LeanTweenType)ieaseType);
	}
